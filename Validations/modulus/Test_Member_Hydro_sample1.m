...
Member.Xt(1)=0;Member.Yt(1)=20;Member.Zt(1)=0;% 关键参数或中间量设置
Member.D=1.0;% 构件直径
for i=1:Num_bar
...
for i=1:N
    ti=(i-1)*dt;
    [Ftx(i),Fty(i),Ftz(i),Mtx(i),Mtz(i)]=Hydro_member1(1,Discrete.Num_ele(1),Discrete.dL(1),ti,0);%Hydro_member_drag_only
end
Ftx_max=max(abs(Ftx));
Fty_max=max(abs(Fty));
Ftz_max=max(abs(Ftz));
