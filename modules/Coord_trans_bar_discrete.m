...
% Rotate all member end coordinates in code X-Z by pesai deg (Y vertical).
% Azimuth source: resolve_structure_azimuth(cfg); see paper_code_mapping (Step 3).
% 本函数用于导管架节点坐标变换、杆件离散；
% 输入参数：
...
if debug==1
    if pesai<0
        error('pesai0Coord_trans_bar_discretepesai');
    end
    if Num_bar<0
        error('Num_bar0Coord_trans_bar_discreteNum_bar');
    end
end
% 主函数：
for i=1:Num_bar                                                            %对杆件编号进行循环
    ...
    if Member.L(i)<0
        error('0Coord_trans_bar_discrete');
    end
