function [Fx,Fy,Fz,Mx,Mz]=Hydro_member_inertia_only(i,N,dt_L,t,pesai)
% 该函数用于计算水下杆件的水动力荷载时程;
% 输入参数：
% - i: 构件编号;
% - N: 结构物的单元数;
...
%     x(j)=Member.X0(i)+(j-0.5)*dt_L*sind(Member.fai_y(i))*sind(Member.cita_x(i));
    ...
    Fjx=(0.25*Hydro.density*pi*(Member.D(i))^2*Hydro.cm*anx(j))*dt_L; %0.5*Hydro.density*Member.D(i)*Hydro.cd*V(j)*un(j)+
    Mjz=Fjx*y(j);
	Fjy=(0.25*Hydro.density*pi*(Member.D(i))^2*Hydro.cm*any(j))*dt_L; %0.5*Hydro.density*Member.D(i)*Hydro.cd*V(j)*vn(j)+
	Fjz=(0.25*Hydro.density*pi*(Member.D(i))^2*Hydro.cm*anz(j))*dt_L;%0.5*Hydro.density*Member.D(i)*Hydro.cd*V(j)*wn(j)+
	Mjx=Fjz*y(j);
    else
        Fjx=0;
        Mjz=0;
