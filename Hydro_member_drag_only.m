function [Fx,Fy,Fz,Mx,Mz]=Hydro_member_drag_only(i,N,dt_L,t,pesai)
% 该函数用于计算水下杆件的水动力荷载时程;
% 输入参数：
% - i: 构件编号;
% - N: 结构物的单元数;
...
%     x(j)=Member.X0(i)+(j-0.5)*dt_L*sind(Member.fai_y(i))*sind(Member.cita_x(i));
    ...
    if y(j)<=(Wave.S+h_surface)
        ...
        Fjx=(0.5*Hydro.density*Member.D(i)*Hydro.cd*V(j)*un(j))*dt_L; %+0.25*Hydro.density*pi*(Member.D(i))^2*Hydro.cm*anx(j)
        Mjz=Fjx*y(j);
        Fjy=(0.5*Hydro.density*Member.D(i)*Hydro.cd*V(j)*vn(j))*dt_L; %+0.25*Hydro.density*pi*(Member.D(i))^2*Hydro.cm*any(j)
        Fjz=(0.5*Hydro.density*Member.D(i)*Hydro.cd*V(j)*wn(j))*dt_L;%+0.25*Hydro.density*pi*(Member.D(i))^2*Hydro.cm*anz(j)
        Mjx=Fjz*y(j);
    else
        Fjx=0;
        Mjz=0;
